pipeline {
    ...
    stages {
        ...
        stage('Install & Test') {
            steps {
                // Installs dependencies and runs "fake" tests so pipeline passes
                sh 'npm install'
        ...
        stage('Deploy to EC2') {
            steps {
                // SSHs into the server, pulls the new image, and restarts the game
